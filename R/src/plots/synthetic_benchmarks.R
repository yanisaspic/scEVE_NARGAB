...
get_lineplots <- function(benchmark, metric, draw_legend) {
  ...
  tmp <- tmp[tmp$n>15,] # do not plot configurations with a minority of observations
  
  # this section is dedicated to aesthetics ____________________________________
  shapemaps <- stats::setNames(ifelse(tmp$method=="scEVE*", 17, 20), tmp$method)
  linetypemaps <- stats::setNames(ifelse(tmp$method=="scEVE*", "dashed", "solid"), tmp$method)
  ...
  plot <- ggplot2::ggplot(tmp, ggplot2::aes(x=k, y=y)) +
    ...
    ggplot2::scale_shape_manual(values=shapemaps, breaks=names(aes_params$colormap)) +
    ggplot2::scale_linetype_manual(values=linetypemaps, breaks=names(aes_params$colormap)) +
    ggplot2::scale_x_continuous(breaks=1:10) +
    ggplot2::scale_y_continuous(expand=ggplot2::expansion(mult=0.05), position="right") +
    ggplot2::scale_color_manual(values=unlist(aes_params$colormap), breaks=names(aes_params$colormap))
...
get_lineplot_legend <- function(benchmark, metric) {
  ...
  #' 
  #' 
  tmp <- get_lineplots(benchmark, metric, draw_legend=TRUE)
...
get_lineplot.synthetic_benchmark <- function(benchmark, metrics) {
  ...
  plot <- ggpubr::ggarrange(plot, get_lineplot_legend(benchmark, metrics[1]), nrow=2, ncol=1, heights=c(10,1))
  return(plot)
}
...
get_plots.synthetic_benchmark <- function(benchmark) {
  ...
  benchmark$dataset <- as.character(benchmark$dataset)
  ...
  plots.synthetic_benchmark <- list(
    ...
    supplementary_clustering_performance=get_lineplot.synthetic_benchmark(benchmark, c("ARI", "nPurity")),
